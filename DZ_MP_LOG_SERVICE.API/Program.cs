...
using DZ_MP_LOG_SERVICE.API.Data;
...
builder.Services.AddScalarOpenApi("DZ-MP Log Service API");
...
builder.Services.Scan(scan => scan
    ...
    .AddClasses(classes => classes.InNamespaces(
        "DZ_MP_LOG_SERVICE.API.Repositories.Impl",
        "DZ_MP_LOG_SERVICE.API.Services.Impl"))
    .AsMatchingInterface()
    .WithScopedLifetime()
);
...
if (app.Environment.IsDevelopment())
{
    ...
    {
        options.Title = "DZ-MP Log Service API";
    });
}
...
app.MapGet("/", () => Results.Ok(new { service = "log-service", status = "running" }))
